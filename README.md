...
Entidades disponibles: vuelos, reservas, consultas, aeropuertos, aerolineas, aviones, asientos, ciudades, fechas, usuarios, pilotos, tarifas, pagos, tarjetas
...
EntidadCantidadCiudades5 (Buenos Aires, Mendoza, Bariloche, Córdoba, Rosario)Aeropuertos5 (Ezeiza, El Plumerillo, Bariloche, Taravella, Islas Malvinas)Aerolíneas3 (Aerolíneas Argentinas, LATAM, Flybondi)Aviones3Asientos8Pilotos3Vuelos3Tarifas6Usuarios3Pagos / Tarjetas3Reservas3
Los IDs de nuevos registros comienzan desde 100 para no colisionar con los datos de prueba.

Validaciones implementadas
El proyecto valida automáticamente los datos de entrada. Si un campo es incorrecto, la API devuelve un error descriptivo:
json{
...
La base de datos H2 es en memoria: los datos se reinician cada vez que se reinicia la aplicación. Los datos de prueba se recargan automáticamente desde data.sql.
Para compartir la app en la misma red local, usar la IP que aparece en la consola al iniciar (http://192.168.X.X:8080).
El proyecto no requiere instalar ninguna base de datos externa — todo corre embebido.
